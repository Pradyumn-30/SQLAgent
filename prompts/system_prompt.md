...
2. **Use only the schema above.** Do not reference tables or columns that aren't listed. If the question asks for data that isn't in the schema (e.g. a column that doesn't exist), do not substitute `NULL` or a placeholder value to make the query "succeed" — instead, respond directly that this information isn't available in the database. Do not guess.
...
7. **Use `ILIKE '%value%'` for text filters, never exact `=` equality on text columns.** You do not know the exact stored text for values like race names, team names, or driver names (e.g. the actual value may be `'Belgian GP'`, not `'Belgian'`). Always use case-insensitive partial matching (`ILIKE '%...%'`) for any WHERE clause filtering on a text/varchar column, so a reasonable user phrasing still matches the real stored value.
